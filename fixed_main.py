...
# -*- coding: utf-8 -*-
"""
基于TradingAgents的A股量化交易系统主程序 - 固定股票列表版本

此版本从 config/fixed_stock.json 读取固定的股票列表进行分析，
而不使用动态股票选择器。其他分析逻辑与 main.py 保持一致。
"""
...
def main():
    ...
    main_logger = logging.getLogger("AShareTradingSystem.Fixed")
    main_logger.info("🚀 ================ A股量化交易系统启动（固定股票列表模式）================")

    print("启动基于TradingAgents的A股量化交易系统（固定股票列表模式）")
    print("="*60)
    print("📌 本模式从 config/fixed_stock.json 读取固定股票列表")
    print("="*60)
    ...
    # 继续第一阶段：股票选择（从固定配置文件读取）
    stock_selection_start = time.time()
    main_logger.info("🎯 开始股票选择（从fixed_stock.json读取）...")

    try:
        # 从config/fixed_stock.json读取股票列表
        fixed_stock_file = Path(__file__).parent / "config" / "fixed_stock.json"
        print(f"正在从 config/{fixed_stock_file.name} 读取固定股票列表...")

        with open(fixed_stock_file, 'r', encoding='utf-8') as f:
            fixed_config = json.load(f)

        # 提取股票代码和名称的元组列表 (symbol, name)
        # 注意: batch_analyze_threaded 需要 List[Tuple[str, str]] 格式
        stock_list = [(stock['symbol'], stock.get('name', stock['symbol']))
                      for stock in fixed_config.get('stocks', [])]

        stock_selection_time = time.time() - stock_selection_start
        main_logger.info(f"✅ 股票选择完成，耗时: {stock_selection_time:.2f}秒")

        # 显示选股信息
        print(f"选股完成，方法: config/fixed_stock.json配置文件")
        print(f"共选择 {len(stock_list)} 只股票")
        main_logger.info(f"📊 选股结果: 方法=config/fixed_stock.json, 数量={len(stock_list)}")

        # 显示股票列表
        print("固定股票列表:")
        for symbol, name in stock_list:
            print(f"  - {symbol}: {name}")
            main_logger.info(f"  📌 {symbol}: {name}")

    except FileNotFoundError:
        print(f"❌ 未找到 config/fixed_stock.json 文件")
        print("请在 config 目录创建 fixed_stock.json 文件")
        main_logger.error("❌ 未找到 config/fixed_stock.json 文件")
        stock_list = []
    except Exception as e:
        print(f"❌ 从 config/fixed_stock.json 读取股票列表失败: {e}")
        main_logger.error(f"❌ 从 config/fixed_stock.json 读取失败: {e}")
        import traceback
        traceback.print_exc()
        stock_list = []
