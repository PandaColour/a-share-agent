...
class MultiRoundBearResearcher:
    """多轮辩论看跌研究员 - 支持基于历史对话的辩论"""
        ...
        try:
            ...
            if ai_config.get('enable_ai_analysis', False):
                ...
                if model_info:
                    ...
                    logger.info(f"🐻 看跌研究员已加载AI模型: {model_info.get('name')} ({model_info.get('type')})")
                else:
                    logger.warning(f"🐻 看跌研究员未找到模型配置: {model_name}")
                    self.ai_model = None
            else:
                logger.info("🐻 AI分析已禁用，看跌研究员使用传统方法")
        except Exception as e:
            logger.error(f"🐻 看跌研究员AI模型初始化失败: {e}")
            self.ai_model = None
    ...
    def create_debate_argument(self, debate_context: Dict) -> str:
        ...
        try:
            ...
            if self.ai_model:
                ...
                logger.debug(f"🐻 看跌研究员AI回应生成成功: {len(response)} 字符")
                return response
            else:
    ...
    def _get_debate_prompts(self) -> Optional[Dict]:
        ...
        try:
            prompts = self.config_manager.get('system_settings.debate_prompts.bear_researcher', {})
            if not prompts:
                logger.warning("🐻 未找到看跌研究员辩论prompt配置")
                return None
    ...
    def _fallback_argument(self, context: Dict) -> str:
        ...
        return f"""🐻 看跌观点 - 第{debate_round}轮：

⚠️ 核心风险：基于当前市场环境和风险分析，{symbol}面临多重下行压力和投资风险。
...
注：此为系统备选回应，建议配置AI模型以获得更专业的分析。"""
    ...
    def _create_fallback_prompt(self, context: Dict) -> str:
        """创建备选prompt"""
        return f"""请作为专业的看跌研究员，针对股票 {context.get('symbol', '')} 进行风险分析，
基于提供的市场数据和历史对话，识别关键风险因素和下跌催化剂。
请重点关注投资风险，并针对可能的看涨观点进行理性质疑。"""
